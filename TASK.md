...
*   **Скрапинг**: Переход с Firecrawl на **Crawl4AI**.
    *   **ВАЖНО**: Crawl4AI должен работать как **отдельный внешний сервис** (Docker container), с которым бэкенд общается по HTTP API. Не встраивать библиотеку скрапинга в основной код бэкенда.
*   **Инфраструктура**: Docker Compose (приложение + БД + Ollama + Crawl4AI).
...
2.  **Backend (Python)**:
    ...
    *   Воркеры для выполнения скаутов.
3.  **Database**: PostgreSQL (или SQLite файл).
4.  **AI Service**: Ollama контейнер (API на порту 11434).
5.  **Scraper Service**: Crawl4AI контейнер (API сервис, работающий как браузер).
...
*   **Цикл агента**:
    ...
    2.  Сгенерировать план поиска (через API Ollama).
    3.  Выполнить поиск и скрапинг (через HTTP запрос к сервису Crawl4AI).
    4.  Проанализировать результаты.
    5.  Сгенерировать саммари.
    6.  Сохранить результат и эмбеддинг.
*   **Инструменты**:
    *   `search_web`: Поиск (можно использовать DuckDuckGo Search API или возможности Crawl4AI).
    *   `scrape_website`: Вызов API Crawl4AI для получения Markdown контента.
...
## 5. Этапы сдачи
1.  Рабочий `docker-compose.yml` с отдельным сервисом для Crawl4AI.
2.  Backend на `aiohttp` с подключением к БД.
3.  Реализованный класс `ScoutAgent` с интеграцией сервисов Ollama и Crawl4AI.
